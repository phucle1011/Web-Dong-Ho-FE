...
function WishlistList() {
  ...
  const [chartKey, setChartKey] = useState(0); // 🔑 ép remount Chart khi dữ liệu đổi
  const limit = 10;
  ...
  const pieOptions = useMemo(
    () => ({
      ...
      dataLabels: {
        enabled: true,
        formatter: (_val, opts) =>
          `${Math.round(correctedPercents[opts.seriesIndex] ?? 0)}%`,
      },
      tooltip: {
        y: {
          formatter: (_val, { seriesIndex }) => {
            ...
            return `${count} lượt (${Math.round(pct)}%)`;
          },
        },
      },
  ...
  const categoriesRecent = useMemo(
    () =>
      sanitizedRecent.map(
        (v) => (v?.variant?.product?.name || "").trim() || "(Không tên)"
      ),
    [sanitizedRecent]
  );
  ...
  const barData = useMemo(
    () => ({
      ...
      series: [
        { name: "Số lượt", data: sanitizedRecent.map((v) => Number(v.favoriteCount || 0)) },
      ],
    }),
  ...
  // Với trục nhỏ (≤10) -> tick nguyên 0..yMaxInt; lớn hơn -> 6 tick
  const tickAmountInt = useMemo(
    () => (yMaxInt <= 10 ? yMaxInt : 6),
    [yMaxInt]
  );
      ...
      tooltip: {
        x: {
          formatter: (_v, { dataPointIndex }) =>
            categoriesRecent[dataPointIndex] || "(Không tên)",
        },
        y: {
          formatter: (val) =>
            new Intl.NumberFormat("vi-VN").format(val) + " lượt",
        },
      },
  ...
  const fetchGroupedWishlist = async (page) => {
    ...
    try {
      ...
      const params = {
        page,
        limit,
        ...(appliedSearchTerm.trim() && { searchTerm: appliedSearchTerm.trim() }),
      };
  ...
  const fetchStatistics = async () => {
    try {
      const [mostRes, recentRes] = await Promise.all([
        axios.get(
          `${Constants.DOMAIN_API}/admin/wishlist/most-favorited?limit=5`
        ),
        axios.get(
          `${Constants.DOMAIN_API}/admin/wishlist/recently-favorited?days=30&limit=5`
        ),
      ]);

      setMostFavoritedVariants(
        mostRes?.data?.status === 200 ? mostRes.data.data || [] : []
      );
      setRecentlyFavoritedVariants(
        recentRes?.data?.status === 200 ? recentRes.data.data || [] : []
      );

      // 🔑 sau khi set dữ liệu, ép remount 2 biểu đồ
      setChartKey((k) => k + 1);
    } catch (error) {
      toast.error(
        `Lỗi khi tải thống kê: ${error?.response?.data?.error || error.message}`
      );
      setMostFavoritedVariants([]);
      setRecentlyFavoritedVariants([]);
      setChartKey((k) => k + 1);
      ...
      <div className="bg-white p-4 shadow rounded-md">
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="border p-4 rounded-md bg-gray-50">
            {sanitizedMost.length ? (
              <Chart
                key={`pie-${chartKey}`} // 🔑 ép remount theo chartKey
                options={pieOptions}
                series={pieSeries}
                type="pie"
                width="100%"
              ...
              <div className="text-center text-gray-500 py-12">
                Chưa có dữ liệu “yêu thích nhiều nhất”.
              </div>
            )}
          </div>

          <div className="border p-4 rounded-md bg-gray-50">
            {sanitizedRecent.length ? (
              <Chart
                key={`bar-${chartKey}`} // 🔑 ép remount theo chartKey
                options={barOptions}
              ...
              <div className="text-center text-gray-500 py-12">
                Chưa có dữ liệu “yêu thích gần đây”.
              </div>
            )}
          </div>
        </div>
                ...
                groupedWishlistItems.map((grp, idx) => (
                  <tr key={grp.user.id} className="border-b hover:bg-gray-50">
                      ...
                      <div className="grid grid-cols-4 gap-1 p-1">
                        {grp.wishlistItems.slice(0, 4).map((item) => {
                          ...
                          return (
                            <div
                              key={item.id}
                              className="border p-1 rounded-md text-center max-w-[120px]"
                            >
                              {prod?.thumbnail && (
                                <img
                                  src={
                                    prod.thumbnail.startsWith("http")
                                      ? prod.thumbnail
                                      : `${Constants.DOMAIN_API}/Uploads/${prod.thumbnail}`
                                  }
                                  alt={prod?.name}
                        ...
                        {grp.wishlistItems.length > 4 && (
                          <button
                            onClick={() => openModal(grp.wishlistItems)}
                    ...
                    <td className="p-2 border text-center">
                      <Link
                        to={`/admin/wishlist/detail/${grp.user.id}`}
